...
board.ondragstart = function (e) {
    ...
    hideMe = e.target;
    e.dataTransfer.setData('board-card', e.target.id);
...
board.ondragend = function (e) {
    ...
    // After a drag update headers
    updateHeaders();
};

var lastEneterd;

// Gets the closest section
board.ondragenter = function (e) {
    ...
    var section = closestWithClass(e.target, 'board-card-section');
...
board.ondragover = function (e) {
    ...
    // TODO: Check that it's not the original section.
    if (closestWithClass(e.target, 'board-card-section')) {
        e.preventDefault();
    }
};
...
// Dropping a card into a section
board.ondrop = function (e) {
    var section = closestWithClass(e.target, 'board-card-section');
    var id = e.dataTransfer.getData('board-card');
    // console.log("id "+id);
    if (id) {
